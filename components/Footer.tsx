    ...
    <footer className="relative bg-forest text-white py-20 px-6">
      <div className="max-w-[1300px] mx-auto">
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          ...
          >
            <h4 className="text-sm font-semibold mb-5 font-body uppercase tracking-wider text-gold">
              Explore
            </h4>
            <ul className="space-y-3">
              {footerLinks.explore.map((link, index) => (
                <li key={index}>
                  <a
                    href={link.href}
                    className="text-sm text-white/60 hover:text-gold transition-colors duration-200 font-body"
                  >
                    {link.name}
                  </a>
                </li>
            ...
            <ul className="space-y-3">
              {footerLinks.social.map((link, index) => (
                <li key={index}>
                  <a
                    href={link.href}
                    className="text-sm text-white/60 hover:text-gold transition-colors duration-200 font-body flex items-center gap-2"
                  >
                    <span>{link.icon}</span>
                    {link.name}
                  </a>
